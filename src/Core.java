...
public class Core {
    ...
    /**
     * The string representing the final position (rank) of the player after
     * passing the finish line
     */
    public String sFinalPosition;

    /**
     * Integer representation of the final position (rank) of the player after
     * passing the finish line
     */
    ...
    /**
     * Used to count the time ticks (1 tick = 50ms, 20 ticks = 1 second) for
     * score update
     */
    ...
    /**
     * Tells the frequency of updates (gameMaxRunTime = X means one update every
     * X ticks)
     */
    ...
    /**
     * Initializes the finite state machine and stores it in fsStates[]. The
     * probability to reach one state is given in such a way that the straight
     * lines are preferred over the curves and appearing/disappeaing road
     * segments. The sum of probabilities must be (at least (for computational
     * reasons); exactly equal to (for logical reasons)) 1 for each state
     */
    ...
    /**
     * "Main" method of the class. It loops until the GUI asked for closure.
     * After each loop, the thread sleeps for 50 ms. In each loop: - The cars
     * are moved - The collisions are processed - The rectangles to display are
     * computed - The player position (rank) is computed - The GUI is asked to
     * perform its update
     */
    public void runGame() {
     ...
     * @return The array of collided zones: +---+---+---+ | 0 | 1 | 2 |
     * +---+---+---+ | 3 | 4 | 5 | +---+---+---+ | 6 | 7 | 8 | +---+---+---+
     ...
     * @return The intersection between the two rectangles in relative
     * coordinates, or null of the two rectangles don't intersect
     */
